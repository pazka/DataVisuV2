    ...
    */
        public void Close()
        {
            ...
            if (OscPacketIO != null && OscPacketIO.IsOpen())
            {
                OscPacketIO.Close();
                OscPacketIO = null;
                System.Console.WriteLine("Closed OSC listener");
            }
        }
            ...
            switch ((char) packet[start])
            {
                ...
                case '#':
                    ...
                    if (bundleString == "#bundle")
                    {
                        ...
                        while (index < length)
                        {
                            int messageSize = (packet[index++] << 24) + (packet[index++] << 16) + (packet[index++] << 8) +
                                              packet[index++];
                            /*int newIndex = */
        ...
        private static int ExtractMessage(ArrayList messages, byte[] packet, int start, int length)
        {
            ...
            foreach (char c in typeTag)
            {
                switch (c)
                {
                    ...
                    case 'i':
                    {
                        int i = (packet[index++] << 24) + (packet[index++] << 16) + (packet[index++] << 8) +
                                packet[index++];
                        oscM.values.Add(i);
                        break;
                    }
